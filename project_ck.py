        ...
        if len(face) == 9:
            ...
            if len(faces) == 10:
                ...
                if np.array_equal(detected_face, front_face) == True:
                    ...
                    cv2.arrowedLine(bgr_image_input, point1, point2, (0, 255, 0), 4, tipLength=0.2)
        ...
        if len(face) == 9:
            ...
            if len(faces) == 10:
                ...
                if np.array_equal(detected_face, front_face) == True:
                    ...
                    cv2.arrowedLine(bgr_image_input, point1, point2, (0, 255, 0), 4, tipLength=0.2)
                    cv2.arrowedLine(bgr_image_input, point3, point4, (0, 255, 0), 4, tipLength=0.2)
                    cv2.arrowedLine(bgr_image_input, point5, point6, (0, 255, 0), 4, tipLength=0.2)
                    cv2.arrowedLine(bgr_image_input, point7, point8, (0, 255, 0), 4, tipLength=0.2)
        videoWriter.write(bgr_image_input)
        cv2.imshow("Output Image", bgr_image_input)
        key_pressed = cv2.waitKey(1) & 0xFF
        if key_pressed == 27 or key_pressed == ord('q'):
...
def detect_face(bgr_image_input):
    ...
    hsv = cv2.cvtColor(bgr_image_input, cv2.COLOR_BGR2HSV)          # Lọc màu
    
    blob_colors = []
    colors_hsv = [('white', [5, 0, 130], [50, 60, 255]),
                    ('orange',[5, 130, 130], [14, 255, 255]),
                    ('green', [60, 115, 125], [75, 255, 255]),
                    ('blue', [95, 100, 130], [135, 255, 255]),
                    ('yellow', [25, 80, 80], [383, 255, 255]),
                    ('red', [[0, 130, 36], [3, 255, 255]], [[178, 254, 254], [179, 255, 255]])]
        ...
        for contour in contours:
            ...
            if 2800 < area < 10000:
                ...
                if len(approx) == 4:
                    ...
                    cv2.rectangle(bgr_image_input, (x,y), (x+w, y+h), (255,0,255), 2, 2)
                    val = (50*y) + (10*x)
    ...
    if len(blob_colors) == 9:
        for i in range(9):
            if blob_colors[i][0] > 170 and blob_colors[i][1] > 150 and blob_colors[i][2] > 150:
                blob_colors[i][3] = 1
                face[i] = 1
            elif blob_colors[i][0] < 223 and blob_colors[i][1] > 55 and blob_colors[i][2] > 27 and np.abs(blob_colors[i][1]-blob_colors[i][2])<30:
                blob_colors[i][3] = 2
                face[i] = 2
                ...
                blob_colors[i][3] = 4
                face[i] = 4
            elif blob_colors[i][2] > blob_colors[i][0] and blob_colors[i][2] > blob_colors[i][1] and blob_colors[i][0] >9 :
                blob_colors[i][3] = 5
                face[i] = 5
            elif blob_colors[i][1] < blob_colors[i][2] and blob_colors[i][0] < blob_colors[i][1] and blob_colors[i][2] > 170:
                blob_colors[i][3] = 6
                face[i] = 6
...
def find_face(video,videoWriter,uf,rf,ff,df,lf,bf,text = ""):
    ...
    while True:
        ...
        face, blob_colors = detect_face(bgr_image_input)
        bgr_image_input = cv2.putText(bgr_image_input, text, (50, 450), cv2.FONT_HERSHEY_SIMPLEX, 2, (248, 110, 75), 3)
        # print(len(face))
        if len(face) == 9:
            faces.append(face)
            if len(faces) == 5:
...
def main():
    ...
    down_face = [0, 0]
    back_face = [0, 0]
    video = cv2.VideoCapture(0)                     # import camera
    is_ok, bgr_image_input = video.read()
    broke = 0
    ...
    while True:
        ...
        while True:
            ...
            while True:
                if (datetime.now() - start_time).total_seconds() > 3:
                    ...
                    if not is_ok:
                        broke = 1
                        break
                    bgr_image_input = cv2.putText(bgr_image_input, "Show Down Face", (50,450), cv2.FONT_HERSHEY_SIMPLEX, 2, (248, 110, 75), 3)
                    videoWriter.write(bgr_image_input)
                    cv2.imshow("Output Image", bgr_image_input)
                    key_pressed = cv2.waitKey(1) & 0xFF
                    if key_pressed == 27 or key_pressed == ord('q'):
            ...
            while True:
                if (datetime.now() - start_time).total_seconds() > 3:
                    ...
                    if not is_ok:
                        broke = 1
                        break
                    bgr_image_input = cv2.putText(bgr_image_input, "Show Right Face", (50, 450), cv2.FONT_HERSHEY_SIMPLEX, 2, (248, 110, 75), 3)
                    videoWriter.write(bgr_image_input)
                    cv2.imshow("Output Image", bgr_image_input)
                    key_pressed = cv2.waitKey(1) & 0xFF
                    if key_pressed == 27 or key_pressed == ord('q'):
            ...
            while True:
                if (datetime.now() - start_time).total_seconds() > 3:
                    ...
                    if not is_ok:
                        broke = 1
                        break
                    bgr_image_input = cv2.putText(bgr_image_input, "Show Left Face", (50, 450), cv2.FONT_HERSHEY_SIMPLEX, 2, (248, 110, 75), 3)
                    videoWriter.write(bgr_image_input)
                    cv2.imshow("Output Image", bgr_image_input)
                    key_pressed = cv2.waitKey(1) & 0xFF
                    if key_pressed == 27 or key_pressed == ord('q'):
            ...
            while True:
                if (datetime.now() - start_time).total_seconds() > 3:
                    ...
                    if not is_ok:
                        broke = 1
                        break
                    bgr_image_input = cv2.putText(bgr_image_input, "Show Back Face", (50, 450), cv2.FONT_HERSHEY_SIMPLEX, 2, (248, 110, 75), 3)
                    videoWriter.write(bgr_image_input)
                    cv2.imshow("Output Image", bgr_image_input)
                    key_pressed = cv2.waitKey(1) & 0xFF
                    if key_pressed == 27 or key_pressed == ord('q'):
            ...
            while True:
                if (datetime.now() - start_time).total_seconds() > 3:
                    ...
                    if not is_ok:
                        broke = 1
                        break
                    bgr_image_input = cv2.putText(bgr_image_input, "Show Front Face", (50, 450), cv2.FONT_HERSHEY_SIMPLEX, 2, (248, 110, 75), 3)
                    videoWriter.write(bgr_image_input)
                    cv2.imshow("Output Image", bgr_image_input)
                    key_pressed = cv2.waitKey(1) & 0xFF
                    if key_pressed == 27 or key_pressed == ord('q'):
            ...
            if (concat(up_face, right_face, front_face, down_face, left_face, back_face) == cube_solved).all():
                is_ok, bgr_image_input = video.read()
                bgr_image_input = cv2.putText(bgr_image_input, "CUBE ALREADY SOLVED", (100, 450), cv2.FONT_HERSHEY_SIMPLEX, 1, (248, 110, 75), 3)
                videoWriter.write(bgr_image_input)
                cv2.imshow("Output Image", bgr_image_input)
                key_pressed = cv2.waitKey(1) & 0xFF
                if key_pressed == 27 or key_pressed == ord('q'):
        ...
        if (concat(up_face, right_face, front_face, down_face, left_face, back_face) == cube_solved).all():
            ...
            is_ok, bgr_image_input = video.read()
            bgr_image_input = cv2.putText(bgr_image_input, "CUBE SOLVED", (100, 450), cv2.FONT_HERSHEY_SIMPLEX, 2, (248, 110, 75), 3)
            ...
            while True:
                if (datetime.now() - start_time).total_seconds() > 5:
                    ...
                    if not is_ok:
                        broke = 1
                        break
                    bgr_image_input = cv2.putText(bgr_image_input, "CUBE SOLVED", (50, 450), cv2.FONT_HERSHEY_SIMPLEX, 2, (248, 110, 75), 3)
                    videoWriter.write(bgr_image_input)
                    cv2.imshow("Output Image", bgr_image_input)
                    key_pressed = cv2.waitKey(1) & 0xFF
                    if key_pressed == 27 or key_pressed == ord('q'):
