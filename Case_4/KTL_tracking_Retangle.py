...
while True:
    ...
    for i,(new,old) in enumerate(zip(good_new, good_old)):
        ...
        frame1 = cv.circle(frame,(int(a),int(b)),5,color[i].tolist(),-1)
        # vuong = cv.rectangle(frame,(int(a),int(b)), (int(a),int(b),int(a)+int(a),int(b)+int(b)), (0,0,255))
    img = cv.add(frame1,mask)
    # update current location
    old_gray = frame_gray.copy()
    p0 = good_new.reshape(-1,1,2)
    
    cv.imshow("first frame", vuong)
    # cv.imshow("first frame", frame1)
